...
def load_agent_brain(agent_name: str, user_id: str = None, current_context: str = "") -> str:
    ...
    # 1.55 PERSONAL HEALTH BRIEF -- Generated by /analyze-health-intake, loaded on every conversation
    health_brief_file = agent_dir / "state" / "HEALTH_BRIEF.md"
    if health_brief_file.exists():
        brain_parts.append(
            f"=== PERSONAL HEALTH INTELLIGENCE BRIEF ===\n"
            f"This brief was generated from this user's intake questionnaire using the full health reasoning framework. "
            f"Use it as the foundational context for all health-related conversations.\n\n"
            f"{health_brief_file.read_text(encoding='utf-8')[:3000]}"
        )
...
def _handle_command_inner(command: str, telegram_config: dict, cmd: str, chat_id: str, bot_token: str):
    ...
    elif cmd.startswith("seed-medical-graph") or cmd.startswith("/seed-medical-graph"):
        # Seed the Neo4j medical knowledge ontology from data/medical_ontology_seed.json
        # Safe to run multiple times (MERGE). Use /seed-medical-graph force to re-seed.
        from core.notification_router import route_notification
        _force_reseed = "force" in cmd
        route_notification(chat_id, f"Seeding medical knowledge graph{'(force)' if _force_reseed else ''}...", bot_token, severity="IMPORTANT", agent="command-center")
        try:
            from core.medical_knowledge_graph import seed_medical_graph, is_seeded
            if not _force_reseed and is_seeded():
                route_notification(chat_id, "Medical graph already seeded. Use /seed-medical-graph force to re-seed.", bot_token, severity="IMPORTANT", agent="command-center")
            else:
                counts = seed_medical_graph(force=_force_reseed)
                if "error" in counts:
                    route_notification(chat_id, f"Seed failed: {counts['error']}", bot_token, severity="IMPORTANT", agent="command-center")
                elif counts.get("skipped"):
                    route_notification(chat_id, "Medical graph already seeded.", bot_token, severity="IMPORTANT", agent="command-center")
                else:
                    route_notification(chat_id,
                        f"Medical knowledge graph seeded\n"
                        f"Conditions: {counts.get('conditions',0)}\n"
                        f"Interventions: {counts.get('interventions',0)}\n"
                        f"Red flags: {counts.get('red_flags',0)}\n"
                        f"Specialists: {counts.get('specialists',0)}\n"
                        f"Relationships: {counts.get('relationships',0)}",
                        bot_token, severity="IMPORTANT", agent="command-center")
        except Exception as e:
            route_notification(chat_id, f"Medical graph seed error: {e}", bot_token, severity="IMPORTANT", agent="command-center")

    elif cmd.startswith("analyze-health-intake") or cmd.startswith("/analyze-health-intake"):
        # Deep health intelligence analysis of a companion's intake questionnaire.
        # Runs once at setup time. Generates a Personal Health Brief stored in state/HEALTH_BRIEF.md
        # Usage: /analyze-health-intake nova
        from core.notification_router import route_notification
        parts = cmd.replace("/analyze-health-intake", "").replace("analyze-health-intake", "").strip().split()
        target_agent = parts[0] if parts else ""

        if not target_agent or target_agent not in CHAT_USER_MAP:
            route_notification(chat_id, f"Usage: /analyze-health-intake <agent>\nValid: {', '.join(CHAT_USER_MAP.keys())}", bot_token, severity="IMPORTANT", agent="command-center")
        else:
            user_id = CHAT_USER_MAP[target_agent]
            agent_dir = AGENTS_DIR / target_agent
            route_notification(chat_id, f"Analyzing health intake for {target_agent} ({user_id})...\nThis may take 30-60 seconds.", bot_token, severity="IMPORTANT", agent="command-center")
            try:
                # Gather all intake files
                intake_parts = []
                for fname in ["AGENT.md", "knowledge.md", "onboarding/QUESTIONNAIRE.md", "onboarding/INTAKE.md"]:
                    fp = agent_dir / fname
                    if fp.exists():
                        intake_parts.append(f"=== {fname} ===\n{fp.read_text()[:4000]}")
                state_ctx = agent_dir / "state" / "CONTEXT.md"
                if state_ctx.exists():
                    intake_parts.append(f"=== state/CONTEXT.md ===\n{state_ctx.read_text()[:2000]}")

                # Load health reasoning framework
                health_reasoning_path = AGENTS_DIR / "shared" / "health-reasoning.md"
                health_framework = health_reasoning_path.read_text() if health_reasoning_path.exists() else ""

                if not intake_parts:
                    route_notification(chat_id, f"No intake files found in {target_agent}/. Add AGENT.md or onboarding/QUESTIONNAIRE.md first.", bot_token, severity="IMPORTANT", agent="command-center")
                else:
                    full_intake = "\n\n".join(intake_parts)
                    analysis_prompt = f"""You are a highly experienced health analyst applying an evidence-based multi-axis decision framework to a new health companion intake.

The person is: {user_id} (agent: {target_agent})

=== HEALTH REASONING FRAMEWORK ===
{health_framework[:8000]}

=== INTAKE DATA ===
{full_intake[:6000]}

Apply the Intake Analysis Protocol from the health reasoning framework. Produce a Personal Health Intelligence Brief with these sections:

1. CONDITION MAP — All conditions (diagnosed + suspected), body systems involved, multi-system overlaps
2. MEDICATION & SUPPLEMENT AUDIT — All current medications, nutrient depletions they cause, supplement interactions to watch for
3. ROOT CAUSE HYPOTHESIS — Most likely unifying upstream drivers (HPA, metabolic, gut-brain, nutritional, nervous system dysregulation)
4. INTERVENTION LADDER POSITION — For each issue, which levels (0-7) have been tried, which haven't
5. PRIORITY ORDER — What to address first, second, third — with evidence rationale
6. FIRST CONVERSATION FOCUS — The 2-3 most important things to explore in the first conversation
7. RED FLAGS — Anything in this intake that triggers escalation concern or needs GP/specialist input now

Be specific, evidence-based, and calibrated. This brief is loaded at the start of every conversation.
Format: clean bullets and sections, no tables, Telegram-readable."""

                    api_key = os.environ.get(f"ANTHROPIC_API_KEY_{user_id.upper()}") or os.environ.get("ANTHROPIC_API_KEY", "")
                    brief_text = call_claude(
                        "You are a health intelligence analyst producing a Personal Health Brief for a companion agent.",
                        analysis_prompt,
                        task_type="deep_analysis",
                        agent_name=target_agent
                    )

                    # Store as state/HEALTH_BRIEF.md
                    state_dir = agent_dir / "state"
                    state_dir.mkdir(exist_ok=True)
                    brief_path = state_dir / "HEALTH_BRIEF.md"
                    brief_path.write_text(f"# Personal Health Intelligence Brief — {user_id}\nGenerated: {datetime.now(NZ_TZ).strftime('%Y-%m-%d %H:%M NZST')}\n\n{brief_text}")

                    # Also seed the brief into user memory so it's in every conversation
                    try:
                        from core.user_memory import extract_and_store_memories
                        synthetic_convo = [
                            {"role": "user", "content": f"This is my complete health brief:\n\n{brief_text[:3000]}"},
                            {"role": "assistant", "content": "I've stored your health brief. I'll use this as the foundation for all our conversations."}
                        ]
                        extract_and_store_memories(user_id, target_agent, synthetic_convo, agent_display_name=target_agent, api_key=api_key)
                    except Exception as _me:
                        logger.warning(f"Memory seeding from health brief failed: {_me}")

                    summary = brief_text[:600] + "..." if len(brief_text) > 600 else brief_text
                    route_notification(chat_id,
                        f"Health brief generated for {target_agent} ({user_id})\nSaved to: agents/{target_agent}/state/HEALTH_BRIEF.md\n\nPREVIEW:\n{summary}",
                        bot_token, severity="IMPORTANT", agent="command-center")
...
System:
  ...
  seed-memory <agent> -- Pre-load agent intake files as memory facts
  clean-memory <agent> -- Remove junk/transient facts from memory
